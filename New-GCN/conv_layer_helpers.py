...
def sparse_dropout(input_tensor, prob, noise_shape):
    ...
    Notes:
        ...
        The `random_tensor` is generated by adding `prob` to uniformly distributed random values in the range [0, 1)
...
def multiply_tensors(x, y, sparse=False):
    ...
    :raises ValueError: If `x` and `y` are not of compatible shapes for matrix multiplication.
